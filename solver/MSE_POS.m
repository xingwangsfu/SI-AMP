function mse_pos = MSE_POS (h_posnega, alpha, delta, epsilon, sigma_e, sigma_e_pos)

syms q_square;

sigma_stat = sqrt(((1+ q_square/delta)^2*sigma_e + sigma_e_pos^2*(1+q_square/delta))/(sigma_e_pos+1+q_square/delta)^2);
...
equ = q_square-mse_soft_thresh;

mse_pos = solve(equ, q_square);
