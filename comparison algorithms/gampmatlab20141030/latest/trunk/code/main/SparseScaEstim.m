...
%                     + py0.*log(max(1e-8,(1-obj.p1))./max(py0,1e-8));
                klDivNeg = py1.*klDivNeg1 + py1.*log(obj.p1./py1) ...
                    + py0.*log((1-obj.p1)./py0);
                ind = find(py1 == 1); %Find indeces where posterior activity is 1
                klDivNeg(ind) = klDivNeg1(ind); %Set those to appropriate value
                klDivNeg(py1 == 0) = 0; %Find indeces where posterior activity is 0
            end
