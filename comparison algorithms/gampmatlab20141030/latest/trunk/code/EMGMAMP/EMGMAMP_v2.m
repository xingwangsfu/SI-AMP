...
%   .maxEMiter          number of EM iterations [default=60]
%   .EMtol              EMGMAMP terminates when norm change between
%                       sparsity rate is less than this tolerance. Set to
%                       -1 to turn off and run max iters. [default= uses
%                       the assumed/learned SNR]
...
%                       variables [default = optEM.heavy_tailed]
...
% Version 5.1
%
function [Xhat, sigmaS, estHist, optEMfin] = EMGMAMP_v2(Y, A, Z, optEM, optGAMP)
...
while stop == 0
    ...
    if ~histFlag
        if t==1
            ...
            estFin = gampEst_v2(inputEst, outputEst, A, Z, sigmaS, optGAMP);
        end
    else
        if t==1
            [estFin,~,estHistNew] = gampEst(inputEst, outputEst, A, optGAMP);
        else
            [estFin,~,estHistNew] = gampEst_v2(inputEst, outputEst, A, Z, sigmaS, optGAMP);
        end
        estHist.step = [estHist.step; estHistNew.step];
        estHist.stepMax = [estHist.stepMax; estHistNew.stepMax];
    ...
    if optEM.learn_noisevar
        if strcmp(optEM.noise_dim,'joint')
            muw = sum(sum(abs(Y-Zhat).^2))/M/T;
            SNRdB = 10*log10( norm(Y,'fro')./muw );
            sigmaS = sum(sum(abs(Z-Xhat).^2))/N/T+sum(sum(Xvar))/N/T;
        elseif strcmp(optEM.noise_dim,'col')
            muw = sum(abs(Y-Zhat).^2,1)/M;
            SNRdB = 10*log10( sum(abs(Y.^2))./muw );
            sigmaS = sum(abs(Z-Xhat).^2,1)/N+sum(Xvar,1)/N;
        elseif strcmp(optEM.noise_dim,'row')
            muw = sum(abs(Y-Zhat).^2,2)/T;
            SNRdB = 10*log10( sum(abs(Y.^2),2)./muw );
            sigmaS = sum(abs(Z-Xhat).^2,2)/T+sum(Xvar,2)/T;
        end
    else
...
if strcmp(optEM.noise_dim,'joint')
    ...
    EMfin.noise_SI = sigmaS(1,1);
elseif strcmp(optEM.noise_dim,'col')
    EMfin.noise_var = muw(1,:);
    EMfin.noise_SI = sigmaS(1,:);
elseif strcmp(optEM.noise_dim,'row')
    EMfin.noise_var = muw(:,1);
    EMfin.noise_SI = sigmaS(:,1);
end
