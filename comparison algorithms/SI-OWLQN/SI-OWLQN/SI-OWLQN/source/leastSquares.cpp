...
LeastSquaresProblem::LeastSquaresProblem(const char* matFilename, const char* bFilename, const char* SIFilename) {
	...
	// proposed
	ifstream SIFile(SIFilename);
	if (!SIFile.good()) {
		cerr << "error opening y-value file " << SIFilename << endl;
		exit(1);
	}
	getline(SIFile, s);
	if (s.compare("%%MatrixMarket matrix array real general")) {
		SIFile.close();
		cerr << "unsupported y-value file format \"" << s << "\" in " << SIFilename << endl;
		exit(1);
	}
	skipEmptyAndComment(SIFile, s);
	stringstream SIst(s);
	size_t SINum, SICol;
	SIst >> SINum >> SICol;
	if (SINum != n) {
		cerr << "number of SI-values doesn't match number of instances in " << SIFilename << endl;
		exit(1);
	} else if (SICol != 1) {
		cerr << "y-value matrix may not have more than one column" << endl;
		exit(1);
...
double LeastSquaresObjective::Eval(const DblVec& input,  DblVec& gradient) {
	...
	for (size_t j=0; j<problem.n; j++) {
		value += (problem.SI[j]-input[j]) *(problem.SI[j]-input[j]) * l2weight;
		gradient[j] = l2weight * (input[j]-problem.SI[j]);
		for (size_t i=0; i<problem.m; i++) {
