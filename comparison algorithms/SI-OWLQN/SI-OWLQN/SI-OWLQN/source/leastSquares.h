...
public:
	LeastSquaresProblem(size_t m, size_t n) : Amat(m * n), b(m),SI(n), m(m), n(n) { }

	LeastSquaresProblem(const char* matfile, const char* bFile, const char* SIFile);
...
//	double Mse (const Db1Vec& input, Db1vec& rec);
