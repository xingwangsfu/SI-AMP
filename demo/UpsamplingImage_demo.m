...
clear all;
% close all;
img_origin = imread('lena.png');
% img_origin=rgb2gray(img_origin); % rgb to grayscale
% img_interp=rgb2gray(img_interp);
img_LR_origin = imresize(img_origin,1/4);
img_interp = imresize(img_LR_origin,4);
[M,N] = size(img_origin);
sigma_trans = 2.5e3;  % variance of Gaussian noise added in the interpolated middle image
...
sigma = 1e3; % variance of measurement noise added in the CS measurements
delta = delta_cand;
